...
const AdminNavbar = ({ isMenuOpen, setIsMenuOpen }: any) => {
  ...
  const isLogged = useAppSelector((state) => state.adminReducer.success);
  ...
  const notifications = useAppSelector(
    (state: any) => state.notificationReducer?.notificationByAdmin.data
  );
  const unreadNotifications = notifications?.filter(
    (note: any) => note.read === false
  );
  const handleSearch = () => {
                ...
                >
                  <div className='text-md cursor-pointer font-semibold h-full flex  justify-center items-center space-x-2'>
                    {adminInfo.logo && (
                      <Image
                        src={
                          process.env.NEXT_PUBLIC_SERVER_HOST + adminInfo.logo
                        }
                        alt='profile'
                    ...
                    <ul>
                      ...
                      <li>
                        <div
                          ...
                          className='rounded-md py-2 z-[10000] hover:bg-gray-600 px-4 w-full flex items-center justify-start cursor-pointer space-x-4 text-white'
                        >
